...
class TournamentBracket(ttk.Frame):
    ...
    def declare_winner(self, match_id, winner_id):
        print(f"Déclaration du vainqueur: Match {match_id}, Équipe {winner_id}")
        db = next(get_db())
        try:
            ...
            if match.bracket_type == 'winner':
                ...
                loser_id = match.team1Id if winner_id == match.team2Id else match.team1Id
                loser_match = (
                    db.query(Match)
                    .filter(
                        Match.tournamentId == self.tournament_id,
