...
class TeamManagement(BaseView):
    ...
    def create_widgets(self):
        ...
        self.tree = ttk.Treeview(main_frame, show='tree headings')
        self.tree['columns'] = ('members', 'teams')
        
        # Configuration des colonnes
        self.tree.column('#0', width=300)  # Colonne pour la hiérarchie
        self.tree.column('members', width=100, anchor='center')
        self.tree.column('teams', width=100, anchor='center')
        
        self.tree.heading('#0', text='Nom')
        self.tree.heading('members', text='Membres')
    ...
    def load_tournaments(self):
        ...
        for tournament in tournaments:
            ...
            tournament_id = self.tree.insert('', 'end', 
                ...
                values=(members_count, teams_count),
                tags=('tournament',))
    ...
    def create_team(self):
        ...
        if not tournaments:
            tk.messagebox.showerror("Erreur", "Aucun tournoi n'existe. Veuillez d'abord créer un tournoi.")
            return
        ...
        ttk.Button(
            buttons_frame,
            text="Créer",
            command=lambda: self.save_team(name_var.get(), tournament_map.get(tournament_var.get()), dialog),
            style='success.TButton'
        ).pack(side='left', padx=5)

    def save_team(self, name, tournament_id, dialog):
    ...
    def edit_team(self):
        ...
        if not selection or 'team' not in self.tree.item(selection[0])['tags']:
            tk.messagebox.showwarning("Attention", "Veuillez sélectionner une équipe à modifier")
            return
        ...
        if not team:
            tk.messagebox.showerror("Erreur", "Équipe non trouvée")
            db.close()
            return
        ...
        ttk.Button(
            buttons_frame,
            text="Enregistrer",
            command=lambda: self.update_team(team.id, name_var.get(), tournament_map.get(tournament_var.get()), dialog),
            style='success.TButton'
        ).pack(side='left', padx=5)
        
        db.close()

    def update_team(self, team_id, name, tournament_id, dialog):
        if not name or not tournament_id:
            tk.messagebox.showerror("Erreur", "Veuillez remplir tous les champs")
            return
    ...
    def delete_team(self):
        ...
        selection = self.tree.selection()
        if not selection or 'team' not in self.tree.item(selection[0])['tags']:
            tk.messagebox.showwarning("Attention", "Veuillez sélectionner une équipe à supprimer")
            return
        
        # Demander confirmation
        if not tk.messagebox.askyesno("Confirmation", "Êtes-vous sûr de vouloir supprimer cette équipe ?"):
            return
